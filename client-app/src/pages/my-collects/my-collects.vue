<template>
  <view class="my-collects-page" :class="themeStore.themeClass">
    <view class="container">
      <!-- 空状态 -->
      <view v-if="!loading && poetryList.length === 0" class="empty-box">
        <text class="empty-icon">⭐</text>
        <text class="empty-text">还没有收藏任何诗词</text>
      ...
      <view v-else class="poetry-list">
        ...
        >
          ...
          <view class="poetry-footer">
            ...
            <button
              class="uncollect-btn"
              @click.stop="handleUncollect(poetry.id)"
            >
              取消收藏
            </button>
          </view>
...
import { getUserCollectedPoetryList, uncollectPoetry, type Poetry } from '@/api/poetry';
...
const loadPoetryList = async (refresh = false) => {
  ...
  try {
    ...
    const response = await getUserCollectedPoetryList({
      page: page.value,
      size: 10,
    });
